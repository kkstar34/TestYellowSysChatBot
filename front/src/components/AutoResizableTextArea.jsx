...
function AutoResizableTextarea() {
  ...
  const handleUpload = async () => {
    ...
    const fileSizeTotal = selectedFile.size / 1000;

    (fileSizeTotal < 1024) ? setFileSize(fileSizeTotal + "KB")  : setFileSize((selectedFile.size / (1024*1024)).toFixed(2) + "MB");
  ...
  return (
    <>
    {progress > 0 ? <ProgressBar fileName={fileName} fileSize={fileSize}/> : null}
      <div className="tokens-container-small">
        <span className="tokens-small">{`Tokens : ${nbTokens}`}</span>
      </div>
      <div className="textarea-container">
        ...
        <div className="button-group">
          <div className="tokens-container">
            <span className="tokens">{`Tokens : ${nbTokens}`}</span>
          </div>
